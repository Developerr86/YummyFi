...
import { getFirestore, collection, onSnapshot, doc, setDoc, addDoc, deleteDoc, serverTimestamp, updateDoc } from "firebase/firestore";
...
const MenuManagement = () => {
    ...
    const handleSaveMenu = async (menuData) => {
        if (editingMenu) {
            ...
            // New menus are not live by default
            await addDoc(collection(db, "menus"), { ...menuData, isLive: false, createdAt: serverTimestamp() });
        }
        setEditingMenu(null);
        setIsCreating(false);
    };

    const handleToggleLiveStatus = async (menuId, currentStatus) => {
        const menuRef = doc(db, "menus", menuId);
        await updateDoc(menuRef, { isLive: !currentStatus });
    };

    const handleDelete = async (menu) => {
        if (window.confirm("Permanently delete this menu?")) {
            if (menu.isLive) {
                alert("You cannot delete a menu that is currently live. Unset it first.");
                return;
            }
    ...
    return (
        <div className="menu-management">
            ...
            <div className="menu-list">
                {menus.map(menu => (
                    <div key={menu.id} className={`menu-list-item ${menu.isLive ? 'live' : ''}`}>
                        ...
                        <div className="menu-item-actions">
                            <button onClick={() => handleToggleLiveStatus(menu.id, menu.isLive)} className="set-live-btn">
                                {menu.isLive ? '✔ Unset Live' : 'Set Live'}
                            </button>
                            <button onClick={() => setEditingMenu(menu)} className="edit-item-btn">✏️ Edit</button>
                            <button onClick={() => handleDelete(menu)} className="delete-item-btn">🗑️ Delete</button>
                        </div>
                    </div>
                ))}
            </div>
